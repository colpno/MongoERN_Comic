...
function InventoryInteract({ cx, handleSort, sortOptions, filterValue, handleFilter }) {
  return (
    <Col className={cx("inventory__general__options")}>
      <Button to="/profile/history/ticket" className={cx("inventory__general__options__redirect")}>
        Lịch sử nhận vé
      </Button>
      <div className={cx("inventory__general__options__select")}>
        ...
        <Select options={sortOptions} value={filterValue} setValue={handleFilter} height={34} />
      </div>
    </Col>
  );
}
