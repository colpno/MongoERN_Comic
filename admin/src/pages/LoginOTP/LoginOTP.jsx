...
//         const formatedTime = moment(expiredTimeInSecond).format("mm:ss").toString();
...
//         ...prev,
//         isShown: true,
//         content: "Bạn sẽ được chuyển hướng quay lại màn hình đăng nhập vì mã OTP đã hết hạn",
//       }));
//     }
//   }, [expiredCountdown]);
...
//     const OTPString = OTPKeys.reduce((str, key) => `${str}${OTPInput[key]}`, "");
...
//         otp: OTPString,
//       };
//       const { email, userGuid: userId } = loginInfo;

//       authService.verifyLogin(userId, username, email, OTPString);
//       loginOTP(data)
//         .then((response) => {
//           if (response) {
...
//     if (keyCode >= UNICODE_OF_0 && keyCode <= UNICODE_OF_9 && target.value.length !== 0) {
//       const digit = currentDigit.getAttribute("data-digit");
...
//                   {loginInfo.email &&
//                     replaceAt(loginInfo.email, 3, loginInfo.email.indexOf("@") - 3, "********")}
//                 </span>
//                 <p>
